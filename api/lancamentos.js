...
const TABLE_NAME_PADRAO = "Lancamentos";

function validarRegistro(r) {
  if (!r || typeof r !== "object") return false;
  if (!r.Nome_Promotor || !r.Nome_Cliente || !r.Dia_Lancamento) return false;
  if (!/^\d{4}-\d{2}-\d{2}$/.test(String(r.Dia_Lancamento))) return false;
  if (r.Valor_Faturamento === undefined || r.Valor_Faturamento === null) return false;
  const valor = Number(r.Valor_Faturamento);
...
module.exports = async function handler(req, res) {
  ...
  if (invalidos.length > 0) {
    res.status(400).json({
      erro: "Um ou mais registros estão incompletos, com data em formato inválido (esperado AAAA-MM-DD) ou com Valor_Faturamento ausente/negativo.",
      invalidos
    });
    return;
  }
  ...
  try {
    ...
    const values = lancamentos.map(r => [r.Nome_Promotor, r.Nome_Cliente, r.Dia_Lancamento, Number(r.Valor_Faturamento)]);

    const driveId = process.env.DRIVE_ID || DRIVE_ID_PADRAO;
    const itemId = process.env.ITEM_ID || ITEM_ID_PADRAO;
    const tableName = process.env.TABLE_NAME || TABLE_NAME_PADRAO;
